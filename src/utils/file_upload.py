...
_client = None  # ← don't initialize here
...
def get_client():
    global _client
    if _client is None:
        _client = storage.Client()  # ← only runs when first actually needed
    return _client
...
async def save_file(file, folder: str):
    try:
        ...
        bucket = get_client().bucket(BUCKET_NAME)  # ← use get_client()
...
def download_from_gcs(gcs_path: str) -> bytes:
    try:
        print("⬇️ downloading from GCS:", gcs_path)
        bucket = get_client().bucket(BUCKET_NAME)  # ← use get_client()
...
def delete_from_gcs(gcs_path: str) -> None:
    try:
        print("🗑️ deleting from GCS:", gcs_path)
        bucket = get_client().bucket(BUCKET_NAME)  # ← use get_client()
        blob = bucket.blob(gcs_path)
        blob.delete()
        print("✅ deleted:", gcs_path)
    except Exception as e:
        print(f"⚠️ GCS delete failed (non-fatal): {e}")